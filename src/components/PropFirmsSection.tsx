    ...
    <>
      ...
      <section className="container mx-auto px-4 py-20">
        ...
        <div className="grid gap-6 sm:grid-cols-2 max-w-4xl mx-auto">
          {propFirms.map((p) => (
            <Card key={p.name} className="group transition-all hover:shadow-lg hover:border-emerald-500/30">
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 flex h-16 w-16 items-center justify-center rounded-xl bg-muted/80 p-2">
                    {p.logo ? (
                      ...
                      <span className="text-lg font-bold text-emerald-500">{p.name.split(" ").map(w => w[0]).join("")}</span>
                    )}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-semibold text-lg">{p.name}</h3>
                      <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-emerald-500 transition-colors">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </a>
                    </div>
                    <Badge variant="outline" className="text-xs text-emerald-600 border-emerald-500/30 mb-2">{p.highlight}</Badge>
      ...
      <section className="border-y bg-muted/30">
        <div className="container mx-auto px-4 py-20">
          <div className="text-center mb-12">
            <Badge variant="secondary" className="mb-4 text-emerald-600">Dokazi isplata</Badge>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 max-w-5xl mx-auto">
            {payoutImages.map((img, i) => (
              <Card key={i} className="overflow-hidden group transition-all hover:shadow-lg hover:border-emerald-500/30">
